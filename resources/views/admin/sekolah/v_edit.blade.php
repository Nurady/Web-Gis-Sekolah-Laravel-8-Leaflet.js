            ...
            <form action="{{ route('sekolah.update', $sekolah->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body"> 
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select 
                                    ...
                                    name="status"
                                >
                                    <option {{ $sekolah->status == 'Negeri' ? "selected" : "" }} value="Negeri">Negeri</option>
                                    <option {{ $sekolah->status == 'Swasta' ? "selected" : "" }} value="Swasta">Swasta</option>
                                </select>
                                @error('status')
                                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="form-group">
                                ...
                                >
                                    @foreach ($jenjang as $item)
                                        <option {{ $sekolah->jenjang_id == $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->jenjang }}</option>                                        
                                    @endforeach
                                </select>
                                @error('jenjang_id')
                                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                            ...
                            <div class="form-group">
                                ...
                                >
                                    @foreach ($kecamatan as $item)
                                        <option {{ $sekolah->kecamatan_id == $item->id ? "selected" : "" }} value="{{ $item->id }}">{{ $item->kecamatan }}</option>                                        
                                    @endforeach
                                </select>
                                @error('kecamatan_id')
                                    <div class="text-danger text-sm mt-1">{{ $message }}</div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="5" class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi">{{ $sekolah->deskripsi }}</textarea>
                        @error('deskripsi')
                            <div class="text-danger text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
